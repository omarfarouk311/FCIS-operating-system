		...
		//now add it to the working set and the page table
		{
			...
			if(isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX))
			{
				...
				if (LIST_SIZE(&(e->ActiveList)) < e->ActiveListSize)
				{
			        ...
			        e->va_to_ptr[hash_fun(PDX(wse->virtual_address),PTX(wse->virtual_address))] = wse;
					e->va_to_list_type[hash_fun(PDX(wse->virtual_address),PTX(wse->virtual_address))] = 0;
				}
				else
				{
					LIST_INSERT_HEAD(&(e->SecondList), wse);
					e->va_to_ptr[hash_fun(PDX(wse->virtual_address),PTX(wse->virtual_address))] = wse;
					e->va_to_list_type[hash_fun(PDX(wse->virtual_address),PTX(wse->virtual_address))] = 1;
				}
#else
	...
	// your code is here, remove the panic and write your code
	{
		...
		if(isPageReplacmentAlgorithmFIFO()) {
			LIST_FOREACH(ptr_WS_element,&(e->page_WS_list)) {
				free_user_mem(e,ptr_WS_element->virtual_address,PAGE_SIZE);
			}
		}
		...
		//removing page tables from memory
		uint32* dir_ptr = e->env_page_directory;
		for(int i = 0; i < 1024; ++i) {
			uint32* table;
			get_page_table(e->env_page_directory,(i << 21),&table);
			if(table){
				kfree((void *)table);
			}
		}

		//removing dir table from memory
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	e->hard_lim = (uint32*)daLimit;
	e->seg_break = (uint32*)daStart;
