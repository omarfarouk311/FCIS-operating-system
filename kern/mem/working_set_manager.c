...
{    //TODO: [PROJECT'23.MS2 - #14] [3] PAGE FAULT HANDLER - Create a new working set element
    ...
    if(LIST_SIZE(&(e->page_WS_list)) < e->page_WS_max_size){
        struct WorkingSetElement*newSet = kmalloc(sizeof(struct WorkingSetElement));
        newSet->virtual_address = virtual_address;
        e->va_to_ptr[hash_fun(PDX(virtual_address),PTX(virtual_address))] = newSet;
        return newSet;
    }
    else{
        panic("not enough place in Working set List");
    }
