...
void test_realloc_block_FF()
{
	...
	int initAllocatedSpace = 3*Mega;
	initialize_dynamic_allocator(KERNEL_HEAP_START, initAllocatedSpace);
	//[3.3] Increased size (relocate - split)
	bool is_correct = 1;
	void* curPtr = (void*) KERNEL_HEAP_START ;
	int tSize = allocSizes[numOfAllocs-1] + allocSizes[numOfAllocs-3] + allocSizes[0];
	startVAs[0] = realloc_block_FF(NULL, allocSizes[numOfAllocs-1] - sizeOfMetaData());
	startVAs[1] = realloc_block_FF(NULL, allocSizes[numOfAllocs-3] - sizeOfMetaData());
	startVAs[2] = realloc_block_FF(NULL, allocSizes[0] - sizeOfMetaData());

	for (short i = 0; i < 3; ++i) {
		short actualSize;
		if(!i)
			actualSize = allocSizes[numOfAllocs-1];
		else if(i == 1)
			actualSize = allocSizes[numOfAllocs-3];
		else
			actualSize = allocSizes[0];
		actualSize -= sizeOfMetaData();

		midVAs[i] = (short *)((uint32)startVAs[i] + actualSize/2);
		endVAs[i] = (short *)((uint32)startVAs[i] + actualSize - sizeof(short));
		*(midVAs[i]) = *(startVAs[i]) = *(endVAs[i]) = i;
	}

	if(startVAs[0] != (curPtr + sizeOfMetaData()) || startVAs[1] != (curPtr + allocSizes[numOfAllocs-1] + sizeOfMetaData())
			|| startVAs[2] != (curPtr + allocSizes[numOfAllocs-1] + allocSizes[numOfAllocs-3] + sizeOfMetaData())) {
		is_correct = 0;
		panic("Actual address not equal expected\n");
	}
	//old metadata
	struct BlockMetaData* val = (struct BlockMetaData*)((uint32)startVAs[1] - sizeOfMetaData());
	//reallocate
	startVAs[1] = realloc_block_FF(startVAs[1], 3*kilo - sizeOfMetaData());
	midVAs[1] = (short *)((uint32)startVAs[1] + (2*kilo - sizeOfMetaData())/2);
	endVAs[1] = (short *)((uint32)startVAs[1] + (2*kilo - sizeOfMetaData()) - sizeof(short));
	//metadata of the new block
	struct BlockMetaData* meta = (struct BlockMetaData*)((uint32)startVAs[1] - sizeOfMetaData());
	//check address
	if(startVAs[1] != (short*)((uint32)startVAs[2] + allocSizes[0])) {
		is_correct = 0;
		panic("Expected address: %p Actual: %p\n", (void*)((uint32)startVAs[2] + allocSizes[0]), startVAs[1]);
	}
	//check metadata
	if(meta->size != 3*kilo || meta->is_free) {
		is_correct = 0;
		panic("New block attributes is wrong\n");
	}
	//check old address' metadata
	if(!val->is_free || val->size != allocSizes[numOfAllocs-3]) {
		is_correct = 0;
		panic("Did not free the old block\n");
	}
	//check the metadata of the remaining memory
	meta = (struct BlockMetaData*)((uint32)startVAs[1] + 3*kilo - sizeOfMetaData());
	if(!meta->is_free || meta->size != 3*Mega - tSize - 3*kilo) {
		is_correct = 0;
		panic("Did not add metadata for the last block\n");
	}
	//[3.4] Increased size (relocate - no split)
	//old metadata
	val = (struct BlockMetaData*)((uint32)startVAs[2] - sizeOfMetaData());
	startVAs[2] = realloc_block_FF(startVAs[2], 6*kilo - sizeOfMetaData());
	midVAs[2] = (short *)((uint32)startVAs[2] + (4*kilo - sizeOfMetaData())/2);
	endVAs[2] = (short *)((uint32)startVAs[2] + (4*kilo - sizeOfMetaData()) - sizeof(short));
	meta = (struct BlockMetaData*)((uint32)startVAs[2] - sizeOfMetaData());
	//check address
	if(startVAs[2] != (short*)((uint32)startVAs[0] + allocSizes[numOfAllocs-1])) {
		is_correct = 0;
		panic("Expected address: %p Actual: %p\n", (void*)((uint32)startVAs[2] + allocSizes[0]), startVAs[1]);
	}
	//check metadata
	if(meta->size != 6*kilo || meta->is_free) {
		is_correct = 0;
		panic("New block attributes is wrong\n");
	}
	//check old address' metadata
	if(val->is_free && val->size) {
		is_correct = 0;
		panic("Did not free the old block\n");
	}
	//check the data in the block is not changed
	for (short i = 0; i < 3; ++i) {
		if(*(midVAs[i]) != i || *(startVAs[i]) != i || *(endVAs[i]) != i){
			is_correct = 0;
			panic("Data in the block changed\n");
		}
	}

	//[4.2] Decrease block size and next is free(coalesce)
	struct BlockMetaData* valPtr = (struct BlockMetaData*)((uint32)startVAs[1] + ((struct BlockMetaData*)((uint32)startVAs[1] - sizeOfMetaData()))->size - sizeOfMetaData());
	midVAs[1] = (short *)((uint32)startVAs[1] + (2 * kilo - sizeOfMetaData())/2);
	endVAs[1] = (short *)((uint32)startVAs[1] + 2 * kilo - sizeOfMetaData() - sizeof(short));
	*(midVAs[1]) = *(startVAs[1]) = *(endVAs[1]) = 1;
	int oldSize = valPtr->size;
	startVAs[1] = realloc_block_FF(startVAs[1], 2*kilo - sizeOfMetaData());

	//check old metadata ZEROing
	if(valPtr->size || valPtr->is_free) {
		is_correct = 0;
		panic("try ZEROing size and is_free of old metadata");
	}

	valPtr = (struct BlockMetaData*)((uint32)valPtr - kilo);
	//check new metadata
	if(valPtr->size != oldSize + kilo || !valPtr->is_free){
		is_correct = 0;
		panic("new metaData attributes incorrect");
	}
	valPtr = (struct BlockMetaData*)((uint32)valPtr - 2*kilo);
	//check the reallocated metadata
	if(valPtr->size != 2*kilo || valPtr->is_free) {
		is_correct = 0;
		panic("reallocated metaData attributes incorrect");
	}
	//check the data in the reallocated block
	if(*(midVAs[1]) != 1 || *(startVAs[1]) != 1 || *(endVAs[1]) != 1) {
		is_correct = 0;
		panic("Data modified");
	}
	//[5] Not enough memory
	initialize_dynamic_allocator(KERNEL_HEAP_START, 10*kilo);
	startVAs[0] = realloc_block_FF(NULL, 3*kilo - sizeOfMetaData());
	startVAs[1] = realloc_block_FF(NULL, 7*kilo - sizeOfMetaData());
	realloc_block_FF(startVAs[0], 0);
	meta = (struct BlockMetaData*)((uint32)startVAs[1] - sizeOfMetaData());
	if(startVAs[1] != realloc_block_FF(startVAs[1], 100*kilo - sizeOfMetaData())) {
		is_correct = 0;
		panic("Allocation successful");
	}
	if(meta->is_free || meta->size != 7*kilo) {
		is_correct = 0;
		panic("Meta data edited");
	}

	//[6.1] meta data does not fit (shrinking block)
	endVAs[0] = startVAs[0] + 3*kilo - sizeOfMetaData() - sizeof(short);
	*(endVAs[0]) = 100;
	struct BlockMetaData* metaDataOf1 = (struct BlockMetaData*)((uint32)startVAs[0] - sizeOfMetaData());
	startVAs[0] = realloc_block_FF(startVAs[0], 3*kilo - 2*sizeOfMetaData());

	if(*(endVAs[0]) != 100 || startVAs[0] != (short*)&(metaDataOf1[1]) || metaDataOf1->size != 3*kilo || meta->size != 7*kilo) {
		is_correct = 0;
		panic("#6.1 untracked memory");
	}

	//[6.2] meta data does not fit (expanding block)
	startVAs[1] = realloc_block_FF(startVAs[1], 5*kilo - sizeOfMetaData());
	startVAs[1] = realloc_block_FF(startVAs[1], 7*kilo - 2*sizeOfMetaData());
	if(meta->size != 7 * kilo || startVAs[1] != (short*)&(meta[1])) {
		is_correct = 0;
		panic("#6.2 untracked memory");
	}
