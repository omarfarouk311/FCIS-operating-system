...
//			cprintf("in trap.c\n");
			 int test = pt_get_page_permissions(faulted_env->env_page_directory,fault_va);
			 //cprintf("fault handler: %p\n", fault_va);
			 if(fault_va>=KERNEL_HEAP_START && fault_va<KERNEL_HEAP_MAX){
//				 cprintf("in trap 1\n");
				 sched_kill_env(curenv->env_id);
			 }
			 if((fault_va <= USER_HEAP_MAX && fault_va >= USER_HEAP_START) && ((test)&(1<<11))==0){
//				 cprintf("in trap 2\n");
				 sched_kill_env(curenv->env_id);
			 }

			if(test&PERM_PRESENT){
	//		 cprintf("%x\n",fault_va);
			 int markedBit = fault_va & (1<<11);
			 if(markedBit == 0){
//				cprintf("%d\n",3);
				sched_kill_env(curenv->env_id);
			 }

			 int writeBit = test & PERM_WRITEABLE;
			 if(writeBit==0){
	//			cprintf("%d\n",4);
				sched_kill_env(curenv->env_id);
