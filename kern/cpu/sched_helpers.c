...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	if(e->env_status != ENV_NEW) {
		e->priority=fix_trunc(fix_sub(fix_int(PRI_MAX),fix_add(fix_div(fix_int(e->recent_cpu),fix_int(4)),fix_mul(fix_int(e->nice_value),fix_int(2)))));
		if(e->priority >= 64) e->priority = 63;
		else if(e->priority < 0) e->priority = 0;
	}
...
int env_get_recent_cpu(struct Env* e)
{
    ...
    return fix_round(fix_mul(fix_int(100), fix_int(e->recent_cpu)));
