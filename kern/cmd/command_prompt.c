...
int process_command(int number_of_arguments, char** arguments)
{
	...
	//Comment the following line before start coding...
     char *t=arguments[0];
	 str2lower(t,arguments[0]);

	 char *cmd_name=arguments[0];
	 int cmd_no_arg=number_of_arguments-1;
	 int no_arg,cnt=0,return_Index=0;

     struct Command *ptr=commands;
	 LIST_INIT(&foundCommands);
	 struct Command_LIST *ptr_toList=&foundCommands;


	 for(int it=0;it<NUM_OF_COMMANDS;it++){

		 char *cur_command_name=(commands+it)->name;
		 str2lower(cur_command_name,(commands+it)->name);
		 no_arg=(commands+it)->num_of_args;

		 int sz=0,j=0,sz_cur_command=strlen(cur_command_name),sz_cmd=strlen(cmd_name);
		 if(sz_cur_command!=sz_cmd){
			 for(int i=0;i<sz_cmd;i++){
				 for(;j<sz_cur_command;j++){
					 if(cmd_name[i]==cur_command_name[j]){
						 sz++;
						 j++;
						 break;
					 }
			 ...
			 if(sz==sz_cmd){
				 LIST_INSERT_TAIL(ptr_toList,&commands[it]);
				 cnt=1;
			 }
		 }
		 else{

			 if(strcmp(cur_command_name,cmd_name)==0&&((no_arg==-1&&cmd_no_arg==0)||(no_arg!=-1&&no_arg!=cmd_no_arg))){
				 LIST_INIT(&foundCommands);
				 LIST_INSERT_TAIL(ptr_toList,&commands[it]);
				 return CMD_INV_NUM_ARGS;
			 }
			 if(strcmp(cur_command_name,cmd_name)==0){
				 LIST_INIT(&foundCommands);
				 return return_Index;
			 }
		 }
