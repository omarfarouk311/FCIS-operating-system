...
void _main(void)
{
	...
	{
		ID_MatOps = sys_create_env("sc_matops", FIFO_WS_Size, 0, 0);
	...
	//Factorial: 1 program
	{
		ID_Fact = sys_create_env("sc_fact_recursive", FIFO_WS_Size, 0, 0);
	...
	{
		ID_Fibs[0] = sys_create_env("sc_fib_recursive", FIFO_WS_Size, 0, 0);
		ID_Fibs[1] = sys_create_env("sc_fib_loop", FIFO_WS_Size, 0, 0);
		ID_Fibs[2] = sys_create_env("sc_fib_memomize", FIFO_WS_Size, 0, 0);
